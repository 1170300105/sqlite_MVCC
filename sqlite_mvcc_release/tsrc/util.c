...
void sqlite3Put3byte(unsigned char * p,u32 v){
  p[0] = (u8)(v>>16);
  p[1] = (u8)(v>>8);
  p[2] = (u8)v;
} 

u32 sqlite3Get3byte(const u8 *p){
  return ((unsigned)p[0]<<16)| (p[1]<<8) | p[2];
}

int sqlte3GetTimeStamp(const unsigned char *p,u32* tStart,u32* tEnd,u8* cid){
    int offset = getVarint32(p,*tStart);
    *tEnd = sqlite3Get3byte(p+offset);
    *cid = p[offset+3];
    return offset + 4;
}
int sqlite3GetTimeStampLen(const u8 *p){
  u32 v ;
  return getVarint32(p,v)+4;
}
/*

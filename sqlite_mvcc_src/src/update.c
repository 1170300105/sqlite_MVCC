...
void sqlite3Update(
  ...
  int iRowidCur;         /*Cursor for get new rowid*/
...
# define isView 0
    ...
    if( chngKey || hasFK>1 || pIdx==pPk
     || indexWhereClauseMightChange(pIdx,aXRef,chngRowid)
      ...
      for(i=0; i<pIdx->nKeyCol; i++){
        if(1||indexColumnIsBeingUpdated(pIdx, i, aXRef, chngRowid) ){
          reg = ++pParse->nMem;
          pParse->nMem += pIdx->nColumn;
  ...
  ** initialize the count of updated rows */
  ...
  if( nChangeFrom==0 && HasRowid(pTab) ){
    sqlite3VdbeAddOp3(v, OP_Null, 0, regRowSet, regNewRowid);
    iEph = pParse->nTab++;
    addrOpen = sqlite3VdbeAddOp3(v, OP_OpenEphemeral, iEph, 0, regRowSet);
    sqlite3OpenTableAndIndices(pParse, pTab, OP_OpenWrite, 0, iBaseCur+1,
                                 aToOpen, 0, 0);
  }else{
  ...
  if( nChangeFrom ){
    ...
    if( HasRowid(pTab) ){
      ...
      ** leave it in register regOldRowid.  */
      ...
      sqlite3VdbeAddOp2(v, OP_NewRowid,iRowidCur, regNewRowid);
  ...
  if( !isView ){
    ...
    */
    ...
    sqlite3CompleteInsertion(
        pParse, pTab, iRowidCur, iIdxCur, regNewRowid, aRegIdx, 
        OPFLAG_ISUPDATE | (eOnePass==ONEPASS_MULTI ? OPFLAG_SAVEPOSITION : 0)|OPFLAG_LASTROWID, 
        1, 0
    );
